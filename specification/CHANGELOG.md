...
Ticket 0302 adds typed document functions beside the exact byte readings for all 21 public structured operations. Each function drives its byte reading once in JSON mode, returns a discriminated success or command-error value, and retains the exact exit and streams. The decoder validates the operation identity, schema version, descriptor-owned result bound, and command-owned stream framing. Login interaction stays opaque. Login and logout preserve only their existing post-mutation synchronization exception. The `home.busy` capability now publishes its exact inclusive 65-byte JSON document limit. Raw readings, command output, side effects, schemas, package paths, and versions do not change.
...
Ticket 0298 makes `bot home show` report the resolved configuration file, runs directory, assemblies directory, installation file, cache directory, and Pi authentication file, together with whether each path exists. JSON adds `data.paths` without changing schema version 1, and Markdown prints the same six readings. The command and its exported reading resolve cache and Pi paths from the caller's environment without loading Pi or reaching the network. A source scan pins Bot's local Pi agent-directory rule to the installed Pi distribution. The result bound rises from 4,096 to 65,536 UTF-8 bytes, so every home path the runtime already admits still fits.

Ticket 0297 lists the home's intelligence table. `bot intelligence list` prints one heading and a pipe table of `name`, `provider`, `model`, and `reasoning`, and `--json` returns the same rows as a newline-terminated version-1 `bot.intelligence.list` document. Rows sort bytewise by name and an unauthored provider reports `-` in Markdown and `null` in JSON. A home with no `config.yaml` lists zero rows. A malformed table refuses with exit 2, code `request-invalid`, and cause `home-invalid`, carrying the reader's faults in `details.faults`. An outside consumer imports `intelligenceListReading` from `bot/admin-readings` and receives the same bytes. The command loads no model runtime and reaches no network.

Ticket 0296 inherits the donor's intelligence on resume. `bot run resume RUN` reads the intelligence name the donor's command rung recorded on its first stage, resolves that name through the current home `intelligences` table, and records `intelligence`, `model`, `reasoning`, and `provider` when the row names one at the `command` rung of the new run. A donor that recorded no command-rung name resolves as before through the assembly, the home, and the defaults. Resume still refuses `--intelligence` and every other command option. The per-stage command option is dropped. The assembly hash identifies what runs, so a command option repointing one stage would leave that hash untouched.
...
Ticket 0288 pins the credential environment registry to Pi's own provider sources. A test scans the pinned Pi package and compares every name it reads with the registry in both directions, so a Pi upgrade that adds, renames, or drops a credential name fails the gate. The scan found eight names Pi reads that the runtime scrubbed nowhere: two AWS regions, three Azure OpenAI endpoint settings, two Kimi OAuth hosts, and the OAuth callback host. Each is an input to authentication and not a secret, so the slots chapter lists them beside the account names and regions it already held. They are scrubbed from a stage and never redacted. Two container token names the AWS SDK reads beneath Pi and five behavior flags Pi reads without any credential stay declared exceptions, each with its reason.

Ticket 0286 scrubs a provider's own report before a reader sees it. A provider that echoes back a value this run supplied under a secret-bearing credential environment name now has that value replaced by a marker naming the variable, on standard error, in the `--json` envelope, and in the run record. The 2,048-byte bound is applied after the replacement, so a cut cannot land inside a credential. The slots chapter now names the recognized variables whose value is an input to authentication rather than a secret — an account name, a project, a region, an identifier, or the path to a credential file. They are scrubbed from a stage exactly as before and their values stay as the provider wrote them, because redacting a region would claim a secret that was never there. The record chapter states the limit beside the fault sentence: the match is by value over the secret-bearing names alone, it misses a credential resolved from `auth.json` or `models.json`, and it cannot establish that a report holds no secret.

Ticket 0283 makes a model failure actionable. Every model refusal now names the model string as authored, the rung it resolved from, what the runtime observed, and one command. A refusal states the catalog it read and nothing past it: a name that catalog does not hold is a name that catalog does not hold, never a retired model and never a model no provider anywhere offers. The sentences that generalised that far are gone.

A model whose provider holds no credential the runtime can see is a new refusal code, `credential-missing`, in the Resolution table. It is decided at admission from the availability the runtime already reads, so the run refuses before it is born rather than dying at its first stage and leaving a record of work that never happened. Its action is `bot auth login <provider>`, which no other code carries. `refusals.md` declares it runtime-only, because a static corpus cannot supply a provider whose credential is absent, and the conformance harness reads that exemption from the chapter. The corpus stays at 143 cases.

Refusals gain "What a refusal carries". A model refusal names the model, the provider, the rung, the cause, and the action beside its sentence, and a run mutation's `--json` refusal array writes all five as fields with `null` where a refusal has no such fact. A cause is drawn from the error envelope's closed vocabulary. Every one of the eight fields carries the 512-byte bound the message has always had, and a field cut to it ends in one ellipsis character. `model-unresolved` settles one fix in two shapes rather than three: a home intelligence row that omits a model never enters the table, so the home read refuses it as `key-missing` naming that row and no node resolves a modelless intelligence.
...
Ticket 0279 makes `bot assembly check` report each reachable flow definition and node once within the fixed ten-child-call ceiling. Named-flow nodes carry their flow identity. Child-only nodes use child option rungs and the schematic `request.<runtime>` source. A selected recursive flow carries separate root and child contexts without duplicate rows. Traversal retains exact call-position and self-depth states for shadowing, ceiling decisions, and depth-dependent artifacts. `child_outputs` and `possible_outputs` preserve artifact alternatives without changing the primary `output` string. Definition rows state the runtime ceiling and authored descent bound. `max-depth` now accepts only safe integers from 1 through 11. Pagination, complete validation, and schema version 1 remain in place.

Ticket 0278 makes run summaries compute verified token consumption across each root and its writer-authorized descendant tree. `tokensStatus` distinguishes complete totals from partial verified prefixes. Missing or invalid roots retain null totals. Malformed turns, unknown provider consumption, child disagreement, ambiguous authorization, and safe-integer overflow cannot produce a complete total. Child agreement opens only a writer-shaped `request.<extension>` basename. Provider reconciliation preserves omitted and explicit repeat identities. Human run lists print exact retained timestamps. Semantic record classification rejects a `run_end` before its `run_start`, and an unsafe non-negative duration remains null. Selected-record event, usage, and show readers keep their existing scope.
