...
A runtime may name two claims for publication 0.1.0 separately:

1. **0.1.0 static conformance** means passing all 143 current model-free corpus cases. It covers only behavior that the corpus can exercise without a model.
2. **0.1.0 invariant compliance** means claiming that live execution satisfies the normative invariants, including behavior the static corpus cannot exercise.

Static corpus passage does not imply invariant compliance. A runtime may report the claims separately; complete 0.1.0 conformance requires both.
...
Request-ingestion tests prove the inclusive 4 MiB bound across direct arguments, complete task sources, retained transformed task bodies, standard input, and resume. They prove maximum-plus-one refusal before run birth, bounded incremental standard-input retention and listener cleanup, source precedence, request-specific held donor reads, and unchanged unlimited raw request retrieval. Ordinary-output tests prove ordered callback settlement, backpressure, callback and error-event races, quiet early close through a direct Node pipe, one bounded non-pipe diagnostic, skipped writes after failure, and preservation of an existing nonzero or signal exit. Existing raw-output tests retain their fixed-extent, mutation, integrity, paused-reader, direct closed-pipe, and Linux `/dev/full` proofs. Portable injected-writer cases cover systems without `/dev/full`.

The permanent platform check runs scratch installation, shipped-example resolution, and the named process, signal, lock, request, output, and cleanup cases on Linux and macOS. It repeats the local Git refusal and injected subflow-signal cleanup fixtures ten times in fresh test processes. The injected fixture proves its scratch cleanup and record state. Separate process and root-signal cases own real child-process, process-group, and root-lock evidence. Platform-admission tests prove that Linux and macOS continue, native Windows refuses every invocation before Bot operation work with WSL guidance, and another platform gets a generic refusal.

Run-list tests prove the inert bounded Markdown and versioned JSON readings, exact retained timestamps, safe non-negative integer millisecond durations, null unsafe durations with both timestamps preserved, reading-clock independence, numeric and null facts, the expanded default and selectable projection order, every reachable shared state-precedence branch, live recordless omission, bounded repeatable typed filters, canonical UTC millisecond timestamp edges, streaming exact count mode across 500 invalid runs, per-cell clipping evidence, bounded warnings with exact omission counts, sparse matches, strict canonical-JSON size-limited path-hashed membership cursors, every named invalid cursor shape, a maximal cursor round trip through a real process, cross-home and cross-filter rejection, creation, removal, and rename conflicts within the traversal, hostile retained and invocation text, home and runs path kinds, explicit-home precedence, structured errors, and complete network-free help. Consumption tests prove a true zero-turn record, root totals, partial readable prefixes, malformed-turn exclusion, exact provider stage identity and ordering, unreconciled gaps, local and cross-record safe-integer boundaries, recursive grandchildren, sequential and fan-out siblings, torn descendant prefixes, unique writer-owned authorization, child identity, request, ending, and chronology agreement, unavailable child states, and selected-root usage scope. Read-observation tests prove count mode skips descendants and request bytes, child agreement opens only the exact admitted request basename, hostile paths stay confined, unrelated child-shaped directories are ignored, and session, output, provider, and other artifact paths are never opened. Record-story tests reject an end before its start and accept equal or later instants.
