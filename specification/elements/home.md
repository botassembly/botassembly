...
`bot home show --home DIR` reads the installation identity without mutation. It requires the explicit option and ignores `BOT_HOME` and the platform default. Markdown is the default. `--json` and `-j` return a newline-terminated version-1 `bot.home.show` document. The `data.paths` object reports `config`, `runs`, `assemblies`, `installation`, `cache`, and `piAuth`. Each entry carries the absolute path Bot resolved and whether it exists at the time of the reading. The Pi session directory is not reported because Bot does not resolve it. Markdown reports the same six paths and standings after `Initialized`. Each result occupies at most 65,536 UTF-8 bytes. An absent show succeeds with `initialized: false` and creates nothing.
