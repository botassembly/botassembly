...
>
> The presentation of install, link, update, and remove commands and the
> representation of provenance may change in a later publication. This chapter
> still specifies the behavior that ships now.
...
[the home](home.md), taking one out, and seeing what is there.
[Inspection](inspection.md) is the lens that reads; this is the one that
writes.
...
home, and `--home DIR` names it ([the home](home.md)). There is no registry
...
`bot assembly list` is also the current structured read command. Its human
rows retain the output shown below. `--json` returns a bounded version-1
`bot.assembly.list` document with assembly identity, kind, provenance or link
target, an opaque continuation cursor, and a summary. It reads no network and
does not change the home. The legacy assembly listing keeps its
listing contract.
...
resolved assembly keeps the ban ([the graph](graph.md)). Removing the link is
...
silently skipped ([refusals](refusals.md)). Nothing is recorded at link time:
...
and reaches no agent ([the graph](graph.md#what-the-tree-may-contain)) — the
...
name in a home with nothing to update ([inspection](inspection.md)).
...
the assembly name and source shown here. `--json` returns one bounded version-1
`bot.assembly.install` document whose data names the assembly, says its kind is
`installed`, and records `changed: true`. Refusals are structured in JSON mode
and never report changed state.
...
`assembly-unknown` ([refusals](refusals.md)), and an install that refuses for
...
name, resolved target, and any `BROKEN` mark. `--json` returns one bounded
version-1 `bot.assembly.link` document whose data names the assembly, says its
kind is `linked`, and records `changed: true`. It reads no network.
...
attempt. `--json` returns one bounded version-1 `bot.assembly.update` document
with ordered outcomes. Every outcome names the assembly, has an `updated`,
`unchanged`, or `failed` state, and carries a bounded reason. A batch stops at
the first failure, exits nonzero, and includes every earlier settled outcome.
Before fetching anything, Bot refuses a selected batch whose worst-case result
would exceed the declared result-byte limit. Mutation results are not paged.
...
the assembly name. `--json` returns one bounded version-1
`bot.assembly.remove` document whose data names the assembly, records whether
the prior installation was `installed` or `linked`, and records
`removed: true`. Missing, unsafe, and in-use targets return structured
refusals in JSON mode.
...
its own copy and not the link ([the record](record.md#what-is-kept-beside-it)),
...
([inspection](inspection.md#bot-run-list)) and can be briefly wrong after a
