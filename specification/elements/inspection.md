...
`bot capabilities` reads the command contracts compiled into the executable and the Bot package's source identity. It reads no home, provider, credentials, network, or cache. Source identity may read the package files and invoke local Git. Markdown is the default. Immediately after its heading, Markdown prints `Runtime: VERSION; source: SOURCE; digest: DIGEST; source tree SHA-256: SHA256`. A null digest prints as `-`. `--json` and `-j` return the same newline-terminated `bot.capabilities` document. Its `data` contains `runtime`, `runtimeSource`, `runtimeDigest`, `runtimeTreeSha256`, and the unchanged `commands` inventory. The result stays at schema version 1. The source values use the same package-version and source-identity resolver as a fresh top-level `run_start`. The source-tree hash identifies the observed package and TypeScript checkout bytes or emitted JavaScript installation bytes on disk. It does not claim to identify modules Node already loaded or an atomic tree state. Missing Git or a non-checkout installation reports source `unknown` and a null digest. An emitted installation always reports that pair, even inside another checkout. A required runtime-tree listing or read failure exits 4 with the common retryable `dependency-failed` result and cause `runtime-identity-unavailable`.
...
`bot intelligence list [--json|-j] [--home DIR]` reads the home's `config.yaml` and lists its `intelligences` table. Home selection uses explicit `--home`, then `BOT_HOME`, then the platform default. It loads no model runtime, reads no provider or credential, and reaches no network. It changes no file.

Markdown prints one heading and a pipe table of `name`, `provider`, `model`, and `reasoning`. An unauthored provider prints `-`. JSON returns one newline-terminated version-1 `bot.intelligence.list` document whose `data` holds one object per row with the same four fields, and an unauthored provider is `null`. Rows sort bytewise by name in both modes. The row named `default` is the one an unnamed intelligence resolves to; no field marks it. A home with no `config.yaml` lists zero rows and exits `0`, printing the heading alone in Markdown and `data: []` in JSON. The complete result is at most 65,536 UTF-8 bytes, and the document is not paged.

A malformed `config.yaml` or a malformed row fails with exit `2`, code `request-invalid`, and cause `home-invalid`, carrying the reader's faults in `details.faults`. Standard output stays empty. A table whose rendered result exceeds 65,536 UTF-8 bytes fails with exit `5`, code `integrity-failed`, and cause `result-oversized`, in both modes. Other malformed requests fail with exit `2` before a reading. JSON requests receive the common structured error. Other requests receive the common bounded inert error.
...
JSON success has `schemaVersion`, `kind`, `data`, `page`, `summary`, and `warnings`. Numeric facts remain numbers and absent facts remain `null`. An ended row's `endedAt` is the exact timestamp from its accepted `run_end`. Its `duration` is the non-negative safe-integer millisecond difference from the accepted `run_start` timestamp to that end timestamp. A non-safe difference produces null while both exact timestamp strings remain available. A `run_end` before its accepted `run_start` makes the record invalid. Bot accepts historical timestamp spellings that `Date.parse` recognizes and preserves the exact strings. Bot does not use the reading clock. A row without an accepted start and end pair carries null for both facts. The result remains `bot.run.list` schema version 1 under the pre-release change-in-place policy. A summary text fact above 1,024 UTF-8 bytes is unavailable rather than truncated and produces a warning. At most 20 warnings appear. `summary.warningCount` names the exact number of warning entries encountered through that response's continuation position and `summary.warningsOmitted` names how many do not appear. One row may contribute record-reading and presentation warnings; neither suppresses the other. A count scans every candidate. Count traversal retains no run rows and uses the same bounded warning set. JSON failure writes one structured error to stderr and nothing to stdout. The error names the failed `run.list` operation, a stable code and cause, a bounded message, retryability, and structured details. Invalid requests exit 2, cursor conflicts exit 3, dependency failures exit 4, and internal integrity failures exit 5. A missing or non-directory home exits 1 with `home-not-found` or `home-invalid`. A missing `runs/` directory is an empty valid home. An unreadable home or non-directory `runs` entry is a dependency failure. Empty valid results succeed.

`tokens` sums valid `turn.total` counts from the root and every recursively accepted child. A valid turn carries non-negative safe integers for all five token counters. A malformed turn is skipped. `tokensStatus` is `complete` only when the root and every authorized descendant are valid ended records, every turn is valid, every provider start has a later matching turn with the same stage identity, no event records unreconciled work, and every addition is safe. An omitted repeat and an explicit repeat of `1` are different stage identities. Otherwise the status is `partial`. Readable incomplete records retain their verified numeric prefix, including zero. Missing, unreadable, unsupported, structurally invalid, or disagreeing roots retain null. An unavailable or disagreeing descendant contributes nothing. Overflow within one record or across descendants produces null. The reader follows each child only from one unique writer-owned `subflow_call` authorization whose exact child path matches its stage, defaulted repeat, retry, and call. It reads that child through the held-file boundary and requires child-record agreement. Before opening retained request bytes, it requires a basename of `request.<extension>` with one through 247 ASCII letters or digits in the extension. It never scans for children. `--count` does not read token or descendant facts. The internal `inspectRuns({ usage: true })` detail remains grouped from the selected root's turns. It does not become a descendant breakdown.

Markdown renders retained text as inert data. `startedAt` and `endedAt` render their exact retained strings. `duration` renders the exact non-negative integer followed by `ms`. Either absent fact renders as `-`. C0 and C1 controls become visible escapes. Backslashes are escaped before pipes, and HTML-significant characters are escaped. Clipping happens after escaping for every field, including `id`. A cell is at most 480 UTF-8 bytes, a row is at most 4,096 bytes, and a page is below 1 MiB. Every clipped cell produces a warning that names the run, field, and omitted escaped-byte count. Human warning diagnostics use the same first 20 warnings, one physical line of at most 1,024 bytes each, followed by one omission line when needed. A human failure is one inert physical line of at most 2,048 bytes and uses the same escaping before clipping.

The command reads run names, bounded root records, and authorized descendant records and retained requests needed for token summaries. It does not read sessions, outputs, provider paths, or other detailed artifacts. State follows one order. A live run directory without a record is omitted. A dead recordless directory is `no-record`. A held-record fault supplies its fault state. A readable record without `run_start` is `incomplete`. A record with `run_end` is `ended`. A remaining started record is `running` or `crashed` according to its lock.
