...
async function invoke(
  argv: string[], where: Place, cwd = where.root, configure?: (boundary: CliBoundary) => void,
): Promise<Invocation> {
...
test("a rejected asynchronous Pi destination preserves the command error contract", async () => {
  const where = await place(), internal = "private resolver failure must not print";
  const result = await invoke(["auth", "import", where.source, "--json"], where, where.root, (boundary) => {
    delete boundary.authPath;
    boundary.authPathResolver = () => Promise.reject(new Error(internal));
  });
  expect(result).toEqual({ code: 4, out: "", err: error("import-write-failed", "dependency-failed",
    "Credential import could not publish Pi authentication.", true) });
  expect(result.err).not.toContain(internal);
  await expect(lstat(where.destination)).rejects.toMatchObject({ code: "ENOENT" });
});
