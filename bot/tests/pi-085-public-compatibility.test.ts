...
test("production imports the coding-agent package only through its public runtime boundary", () => {
  ...
  const imports = readdirSync(source, { recursive: true })
    ...
    .filter((path) => readFileSync(join(source, path), "utf8").includes("@earendil-works/pi-coding-agent"));
  expect(imports).toEqual(["model-runtime.ts"]);
  const boundary = readFileSync(join(source, "model-runtime.ts"), "utf8");
  expect(boundary).toContain('loadedPiModelRuntime ??= import("@earendil-works/pi-coding-agent")');
  expect(boundary).not.toMatch(/@earendil-works\/pi-coding-agent\//u);
});
