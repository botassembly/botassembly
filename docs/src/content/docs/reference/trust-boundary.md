...
description: "The authority Bot gives trusted assemblies, and where to contain work that needs isolation."
sidebar:
  order: 8
---
...
Every stage model receives read, write, edit, and Bash tools. Direct file tools accept absolute paths. Commands, their configuration and aliases, anything they spawn, hooks, and gates run with the operator's filesystem and network authority. The record retains direct tool calls reported by the model harness. It does not watch the filesystem or claim a complete list of changes. Supply your own operating-system or container containment before running an untrusted assembly or model.
