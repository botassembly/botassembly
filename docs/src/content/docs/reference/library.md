...
The package does not promise CommonJS `require`, undeclared subpaths, direct `dist` or `types` subpaths, source files in the package, source maps, npm-registry publication, parsed-object returns, or compatibility with TypeScript versions other than the pinned 5.9.3 check.
