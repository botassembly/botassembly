...
Record shape 1 has a closed event-name vocabulary. The current writer names are `run_start`, `run_end`, `stage_carried`, `stage_start`, `prompt`, `stage_end`, `unreconciled`, `tmp_teardown`, `provider_start`, `turn`, `provider_retry`, `provider_transport`, `gate_start`, `check`, `tool_call`, `subflow_call`, `chose`, `loop_done`, `parallel_done`, `fanout_start`, `fanout_done`, `hook`, `hash_drift`, and `signal`. A shape-1 reader also recognizes the retired `tool_denied` event. It rejects every other unknown event name. It permits unknown fields on a known event because additive fields remain compatible within the shape. Known fields must retain their defined types and relationships.
...
| `tool_denied` (historical reader only) | stage identity, tool, and boundary | current writers never emit this event |
