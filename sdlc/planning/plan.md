...
31. **Planned:** give `run.output` and `run.request`'s fault-to-bytes rule an exported home apart from the module-private `copySelected`, then export both through the `run-readings` door ticket 0291 opened.
