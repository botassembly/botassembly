...
they live one level up in `sdlc/project/` as five project-lifecycle scripts
(`tasks`, `before`, `success`, `failure`, `health`), copied verbatim from the
canonical set. This folder is only what is genuinely this project's.
...
| `examples`| every assembly under `examples/` resolves under `bot assembly check`. Called by `lint` |
...
the lifecycle owner's health check replaced the script.
