...
- Ending production size: 19674 nonblank lines
- Production ceiling: 19674 nonblank lines. The implementation stayed below the accepted 19708-line maximum, so the ratchet uses the measured total.
...
- Hosted diagnostic: branch workflow run 35053342953 passed Ubuntu and failed macOS with the signal message but empty details. That path proves the immediate SIGTERM failed, rather than the grace SIGKILL. The ticket branch now retains the signal phase, bounded errno, and lifecycle flags through grace. A second branch-only workflow dispatch will expose Darwin's exact result before the contract changes again.
- Code review: pending.
