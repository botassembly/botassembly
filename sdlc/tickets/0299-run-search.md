...
Rows order by Bot's `bytewise` comparison of UTF-8 relative-path bytes and then numeric line. Pass the same bytewise-sorted explicit operand list to both tools. Do not ask either tool to sort: ripgrep's path comparator is not Bot's byte comparator, and tool-side sorting may reorder explicit operands. Ripgrep runs with one thread, so it processes the already sorted explicit operands sequentially. Real ripgrep on Linux and macOS must preserve that supplied operand order. The parser enforces a known candidate path, nondecreasing candidate ordinal, and a strictly increasing line within one file. Reordered, repeated, or regressing output is a dependency failure. Tool ordering never selects the page silently.

A page collects `limit + 1` rows. The extra row proves another page and is not returned. `page.next` is an opaque base64url cursor carrying schema version 1, SHA-256 of the resolved absolute home string, SHA-256 of the exact query bytes, the last returned relative file, and its line. The cursor has the existing 8,192 encoded-byte and 6,144 decoded-byte limits. Continuation excludes earlier files and matched lines through that position in the cursor file. A different resolved home or query causes `cursor-selection`; a missing cursor file or impossible line causes `cursor-position`. Both use code `cursor-conflict` and exit 3. Malformed and oversized cursors use code `cursor-invalid`, causes `cursor-malformed` and `cursor-limit`, and exit 2.
...
The first of the absolute operation deadline, an abort, a stream or protocol failure, or the `limit + 1` complete hit activates one termination path. Send `SIGTERM` to the process group once, continue draining both pipes, allow 250 ms, then send `SIGKILL` to the group if it remains. Dispose both local pipe endpoints after the forced kill. Await `exit`, `close`, and pipe settlement for one final 1,000 ms cleanup deadline. A cleanly settled deliberate page stop is success; the other triggers are failures. A missing settlement event, disagreeing status, or cleanup expiry uses `close-failed`. Signals reach descendants that remain in the spawned process group. A tool can escape its group; an escaped descendant cannot keep Bot waiting after pipe disposal, but Bot does not claim to terminate or contain it. Publish the work deadline, grace, cleanup deadline, group behavior, stream total, and protocol-line bound in the descriptor.
...
| Both tool names are absent, a present probe fails, spawn fails, a tool exits unexpectedly, emits stderr, or violates framing or order | `dependency-failed` | `dependency-failed` | true | 4 |
...
| Child status disagrees or termination does not settle | `dependency-failed` | `close-failed` | true | 4 |
...
1. Real ripgrep finds a literal containing regex punctuation in a root record, a child record, and two sessions. Rows have the specified metadata and bytewise path/line order. A repeated literal on one line produces one hit. Regex interpretation would change the fixture's answer. The Linux and macOS proofs pass explicit candidates such as `a+/record.jsonl` and `a/b/record.jsonl` in Bot bytewise order and require real ripgrep with `--threads 1` to preserve that operand order. The fixture also demonstrates that a tool-side path sort can choose a different order, so the invocation carries no sorting option. A controlled ripgrep that reverses files, regresses lines, or repeats a row fails instead of selecting a different page.
