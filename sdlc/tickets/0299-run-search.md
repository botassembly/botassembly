...
- Hosted diagnostic: the next macOS rerun still failed on the single grace SIGKILL. The ticket branch now reports the bounded errno and lifecycle flags for that signal error. A branch-only workflow dispatch will determine whether Darwin reports an already-gone group or a true signaling failure before the contract changes again.
- Code review: pending.
