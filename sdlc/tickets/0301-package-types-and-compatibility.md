...
- Code review response: package build, pack, self-reference, tarball installation, and cleanup now operate in one isolated scratch copy. Public-reader parity tests build and remove their own emitted package copies and pass from a checkout with no `dist` or `types`. The documentation check derives paths from explicit `auth.list` and `model.list` exclusions, the unavoidable `run.session` exception, mutation ownership, and the remaining run and administrative contracts. Structural regression probes fail on the former shared-output mutations and 25 copied eligible operation names, then pass after the changes.
...
- Hosted npm-pin red evidence: exact-SHA runtime run 35082931679 passed Linux, macOS, and WSL platform checks, then the Ubuntu complete check stopped before the package proof because Node 22.22.0 supplied npm 10.9.4 while the proof requires 10.9.8. The workflow contract now requires the check job to install exact npm 10.9.8 after Node setup and rejects an unpinned replacement. The focused workflow check passes 4 tests and the package proof passes all 8 cases after the correction.
- Clean-output regression evidence: without checkout `dist`, the complete gate first failed 16 library comparisons and one outside-reader test with `ERR_MODULE_NOT_FOUND`. Their isolated emitted-package fixtures then passed all 45 focused tests. The repeated complete gate passes 161 script tests and 1,901 Bot tests across 226 files with no shared generated package output.
- Code review: pending.
