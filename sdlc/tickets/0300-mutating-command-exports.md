...
6. Child tests use an injected clock and process seam to pin the 1,000 ms TERM grace and 1,000 ms final settlement deadline. Cover natural success of unlimited duration, numeric nonzero exit, abort settled by handled SIGTERM, direct SIGTERM before run birth, SIGKILL escalation, TERM and KILL `ESRCH`, each signal failure, already-aborted input, spawn error, stdin write and close errors, stdout and stderr errors, exit/close disagreement, a missing exit, a missing close, a descendant holding each pipe, final deadline, endpoint disposal, and listener and timer cleanup. Assert one TERM, at most one KILL, no group signal, and one rejection after cleanup for every transport failure. Every injected and real test child and descendant is removed before the test returns.
...
- Final level: 3.
- Reasons: the ticket adds a public function family and fixes its arguments, environment, prompts, streams, cancellation, bounded escalation, and transport behavior. Correctness spans durable home and credential changes plus a child that may fail while mutation continues. Exact command bytes, outer rejection parity, outside-package imports, failure injection, and three platforms prove it. A wrong wrapper is user-visible and can repeat a mutation, but existing handlers retain their overwrite, lock, owner, and credential protections.
