...
The first of the absolute operation deadline, an abort, a stream or protocol failure, or the `limit + 1` complete hit activates one termination path. Send `SIGTERM` to the process group once and continue draining both pipes. Darwin can return `EPERM` after the direct child and its group have departed. Treat that result as tentative, never signal that process-group identifier again, and require agreeing `exit` and `close` plus EOF and close from both pipes within 1,000 ms. Another TERM error fails with `close-failed` and bounded details containing its admitted errno, stop reason, and direct-process and pipe event flags. After successful TERM, allow 250 ms, then send `SIGKILL` to the group exactly once. `ESRCH` proves the group is already gone. Success proves the uncatchable signal was delivered. Never use signal 0 as a cross-platform survival probe and never signal the process-group identifier after the forced kill, because the operating system may reuse it. Dispose both local pipe endpoints after the forced kill and allow one final 1,000 ms for agreeing `exit`, `close`, and pipe settlement. The cleanup deadline observes only those required events. A cleanly settled deliberate page stop is success; the other triggers are failures. A missing settlement event, disagreeing status, signal error, or cleanup expiry uses `close-failed`. Signals reach descendants that remain in the spawned process group. A tool can escape its group; an escaped descendant cannot keep Bot waiting after pipe disposal, but Bot does not claim to terminate or contain it. Publish the work deadline, grace, cleanup deadline, group behavior, stream total, and protocol-line bound in the descriptor.
...
4. Injected process tests cover natural success, natural no-match, spawn error, abort, absolute timeout despite continuing output, early-page termination, post-stop stream bytes before direct close, SIGTERM grace, exactly one unconditional SIGKILL, ESRCH and success at grace, a grace signal error, cleanup timeout without another signal, probe signal and cleanup failures, a descendant holding each pipe, an escaped descendant holding each pipe, exit/close disagreement, missing exit, missing close, stream failure, and pipe disposal. Every case returns inside its outer bound. Escaped descendants are test-owned and removed by the fixture.
...
- Accepted-ticket baseline: 19258 nonblank lines
- Starting production size: 19674 nonblank lines
- Ending production size: 19682 nonblank lines
- Production ceiling: 19682 nonblank lines. The implementation stayed below the accepted 19708-line maximum, so the ratchet uses the measured total.
...
- Second hosted macOS repair: the rerun still returned generic dependency failure at the grace boundary. The remaining platform-dependent operation was signal 0 on the process group. Settlement now uses one unconditional SIGKILL after grace. Signal failures report `close-failed` with a process-group message. Cleanup waits only for direct process and pipe events and never risks signaling a reused process-group identifier. The hosted macOS rerun remains the proof against real BSD grep and Darwin process groups.
- Hosted diagnostic: branch workflow run 35053804122 passed Ubuntu and exposed Darwin's exact result: immediate TERM returned `EPERM`, while `exit`, `close`, both EOF events, and both stream-close events had settled by the cleanup decision. The runtime now treats only that TERM result as a departed-group observation, never signals the identifier again, and accepts a page only after every required event agrees. Incomplete settlement still fails at the bounded cleanup deadline with the recorded signal state. Another branch-only workflow dispatch owns the real Darwin proof before integration.
- Code review: rejected and remediated across four rounds before the first acceptance. The reviewer required bounded descendant settlement, shared probe bounds and admitted causes, fail-closed protocol validation, portable real-tool proofs, post-stop output rejection, and removal of signal-zero process-group probes. Hosted macOS then exposed two Darwin-only settlement failures after that acceptance. Branch diagnostics showed that BSD grep had fully exited and closed both pipes while the first group `SIGTERM` returned `EPERM`. The final repair preserves the original page, timeout, or abort reason, never signals that process-group identifier again, requires agreeing direct exit and close plus EOF and close from both pipes, and fails incomplete settlement at the bounded cleanup deadline. The same reviewer accepted commit `0c6572a` after 71 focused tests, all 1,846 runtime tests, typecheck, lint, the 19,682-line ratchet, and the clean diff passed.
