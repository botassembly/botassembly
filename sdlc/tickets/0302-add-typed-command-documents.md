...
5. Extend the actual-tarball TypeScript consumer and library reference for the new values and types. Amend only the generic JSON-error paragraph in `specification/elements/inspection.md` to point to the already normative login and logout `synchronization-failed` stdout exceptions in `specification/elements/auth.md`; republish the generated specification page and add a changelog entry. Change no schema, authentication rule, command output, command side effect, package path, runtime dependency, or version.
