...
For either authentication policy, first validate any stdout document and the permitted diagnostic prefix. On exits 1 through 5, when stderr ends in a valid final error envelope for that same operation, return the `error` arm and preserve the prefix byte for byte. Stdout must then be empty except for `synchronization-failed`, where it must be that operation's valid completion document; the result still uses the `error` arm and retains both streams. When no final envelope exists, a valid expected stdout document uses the `document` arm at any exit and permitted stderr remains only in `command`. No other operation or error cause receives the dual-stream rule. Missing terminal newline, wrong or over-bound prefix or envelope, trailing bytes after the envelope, a document on any other refusal, a cross-operation document or error, or a synchronization refusal without its matching document is an invariant failure with a bounded message containing no command bytes.
...
4. Authentication-policy cases prove login success with interaction stderr, login interaction followed by provider refusal, logout success with the exact advisory stderr, and each operation's dual-stream post-mutation synchronization refusal. Every case keeps exact stdout and stderr in `command`; ordinary refusals return only their final compact envelope as `error`; synchronization refusals return the error arm and retain the matching valid stdout document. For both operations, reject a wrong-operation final envelope, a non-synchronization refusal with stdout, a cross-operation success document, and a synchronization refusal without stdout. For login, also reject an over-bound prefix or envelope, a nonterminal envelope, and JSON-shaped interaction text. For logout, reject any prefix other than empty or the exact 80-byte advisory, a second advisory, interaction-sized text, and an invalid dual-stream combination. Every invariant message is bounded and contains no command bytes.
...
7. Existing byte-parity, mutation lifecycle, package-content, declaration, private-export, and command-output tests remain unchanged and pass. The library page distinguishes exact byte readings from typed documents and keeps the pre-1.0 compatibility warning.
