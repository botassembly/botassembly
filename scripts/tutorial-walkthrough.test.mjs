...
test('a clean local clone can materialize the tutorial blocks and check them offline', async () => {
	...
	try {
		...
		const scanner = join(clone, '.tools', 'gitleaks', '8.30.1', 'gitleaks');
		await mkdir(dirname(scanner), { recursive: true });
		await cp(join(repository, '.tools', 'gitleaks', '8.30.1', 'gitleaks'), scanner);
		await chmod(scanner, 0o755);
		run('sh', ['sdlc/scripts/install'], { cwd: clone });
